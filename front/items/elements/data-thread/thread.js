    ...
    render: function()
    {
        ...
        return /* html */ `
            <div :class="'box bg-' + background">
                <e-admin-status-empty ot-if="!entries.length" :icon="emptyIcon" :title="emptyTitle" :description="emptyDescription" :background="0"></e-admin-status-empty>
                <div ot-for="entry in entries" :ot-key="entry.id" :class="state(entry)">
                    ...
                    <e-admin-form-textarea ot-if="_reply && entry.replyable && !entry.reply" :placeholder="replyPlaceholder" :rows="2" :maxlength="2000" :counter="false" :background="above()" :_enter="replier(entry.id)"></e-admin-form-textarea>
                </div>
                <div ot-if="_send" class="composer">
                    <e-admin-form-textarea :placeholder="placeholder" :rows="2" :maxlength="2000" :counter="false" :background="above()" :_enter="sender()"></e-admin-form-textarea>
                </div>
            </div>
        `;
    }
