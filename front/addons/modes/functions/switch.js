$ot.ui.modes.Fn('switch', function(id)
{
	...
	{
		const siblings = Object.values($ot.ui.screens.Items()).filter((candidate) => candidate.Get('app') === screen.Get('app') && candidate.Get('mode') === id);
		const next = siblings.find((candidate) => candidate.Get('isDefault')) || siblings[0];

		if(next)
		{
			$ot.ui.screens.Fn('open', next.Get('id'));
		}
