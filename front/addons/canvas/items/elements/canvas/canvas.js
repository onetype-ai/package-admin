elements.ItemAdd({
	...
	render: function()
	{
		...
		const refresh = () =>
		{
			let items = $ot.ui.canvas.Fn('list');

			if(this.layout === 'stack')
			{
				items = $ot.ui.canvas.Fn('stack', items);
			}
			...
			this.regions = this.layout === 'free' ? $ot.ui.canvas.Fn('regions', items) : [];
			this.edges = $ot.ui.canvas.Fn('edges', items);
			this.map = minimap(items);
			this.frame = bounds(items);

			$ot.ui.canvas.StoreSet('placed', items);
		};
		...
		/* Tidy glides the cards into place, the class arms a transform transition
		   for the re-render that follows on $ot.ui.canvas.move. */
		...
		const measure = () =>
		{
			...
			$ot.ui.canvas.StoreSet('viewport', { width: size.width, height: size.height });
		};
		...
		const keys = (event) =>
		{
			...
			if(this.focus)
			{
				if(event.key === 'ArrowRight' || event.key === 'ArrowLeft')
				{
					...
					const next = $ot.ui.canvas.Fn('near', this.focus, event.key === 'ArrowRight' ? 1 : -1);
		...
		this.render = (entry) =>
		{
			return $ot.ui.canvas.ItemGet(entry.id).Fn('render');
		};
		...
		const live = (moved) =>
		{
			$ot.ui.canvas.Fn('edges', moved).forEach((edge) =>
			{
			...
			$ot.ui.canvas.Fn('regions', moved).forEach((region) =>
			{
		...
		const haul = (event) =>
		{
			...
			const move = (moving) =>
			{
				...
				{
					return $ot.ui.canvas.Fn('collides', {
						id: member.id,
		...
		this.down = (entry) =>
		{
			return ({ event }) =>
			{
				...
				const move = (moving) =>
				{
					...
					const blocking = $ot.ui.canvas.Fn('collides', { id: entry.id, x: candidate.x, y: candidate.y, width: entry.width, height: entry.height });
