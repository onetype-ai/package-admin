commands.Item({
	id: 'ui:canvas:move',
	exposed: true,
	description: 'Move a canvas item to a new position. Persists the layout and emits $ot.ui.canvas.move. Positions apply in the free layout, the stack layout computes its own.',
	...
	{
		const item = $ot.ui.canvas.ItemGet(properties.id);
		...
		const blocking = $ot.ui.canvas.Fn('collides', { id: properties.id, x: properties.x, y: properties.y, width: item.Get('width'), height: item.Get('height') });
