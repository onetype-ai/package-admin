commands.Item({
	...
	callback: async function(properties, resolve)
	{
		...
		const { positions, source } = await $ot.ui.canvas.Fn('arrange', items);
		const ids = Object.keys(positions);

		ids.forEach((id) =>
		{
			const item = $ot.ui.canvas.ItemGet(id);
		...
		const level = $ot.ui.canvas.Fn('frame', items.map((item) => ({ ...item, ...positions[item.id] })));
