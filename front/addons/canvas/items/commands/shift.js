commands.Item({
	...
	callback: function(properties, resolve)
	{
		...
		if(properties.group)
		{
			if(!$ot.ui.canvas.Fn('groups')[properties.group])
			{
				return resolve(null, 'Canvas group ' + properties.group + ' not found.', 404);
			}

			members = Object.values($ot.ui.canvas.Items()).filter((item) => item.Get('group') === properties.group);
			label = 'Canvas group ' + properties.group;
		}
		else if(properties.ids && properties.ids.length)
		{
			const missing = properties.ids.find((id) => !$ot.ui.canvas.ItemGet(id));
			...
			members = properties.ids.map((id) => $ot.ui.canvas.ItemGet(id));
		...
		{
			const blocking = $ot.ui.canvas.Fn('collides', {
				id: item.Get('id'),
