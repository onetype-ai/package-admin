	...
	admin.dashboard = onetype.Addon('admin.dashboard', (addon) =>
	{
		...
		addon.widgets = onetype.Addon('admin.dashboard.widgets', (addon) =>
		{
			...
			addon.Field('type', {
				type: 'string',
				required: true,
				description: 'What the widget renders as. Matches the id of a registered admin.dashboard.types item. Third parties register new types, the shipped ones are numbers, line, bar, list, donut, table, actions, gauge, progress, status and timeline.'
			});
