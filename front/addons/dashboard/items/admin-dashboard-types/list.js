onetype.AddonReady('admin.dashboard.types', function(types)
{
    types.Item({
                    ...
                    rows: {
                        ...
                        each: {
                            ...
                            config: {
                                ...
                                color: {
                                    type: 'string',
                                    description: 'Bar and value color name. Empty uses the widget accent.'
                                },
                                onClick: {
                                    type: 'function',
                                    description: 'Called with the row on click.'
                                }
                            }
                        },
        ...
        render: function()
        {
            ...
            this.Compute(() =>
            {
                this.list = this.payload.rows.map((row) => ({ ...row, color: tone(row.color) }));
            });

            return '<e-admin-data-list :background="0" :rows="list"></e-admin-data-list>';
        }
    });
});
