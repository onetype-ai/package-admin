onetype.AddonReady('admin.dashboard.types', function(types)
{
    types.Item({
                        ...
                        each: {
                            ...
                            config: {
                                ...
                                value: {
                                    type: 'string',
                                    value: '',
                                    description: 'Value readout on the right, like 12 / 32 GB.'
                                },
                                percent: {
                                    type: 'number',
                                    value: 0,
                                    description: 'Fill of the track, 0 to 100.'
                                },
                                color: {
                                    type: 'string',
                                    value: '',
                                    description: 'Bar color name. Empty uses the widget accent.'
                                }
                            }
                        },
        ...
        render: function()
        {
            ...
            this.fill = (bar) =>
            {
                const width = Math.min(Math.max(bar.percent, 0), 100);
                const start = 'var(--ot-' + bar.color + '-hover)';
                const end = 'var(--ot-' + bar.color + ')';

                return 'width: ' + width + '%; background: linear-gradient(90deg, ' + start + ', ' + end + ')';
            };
