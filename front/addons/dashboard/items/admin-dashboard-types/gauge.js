onetype.AddonReady('admin.dashboard.types', function(types)
{
    types.Item({
        ...
        config: {
            ...
            payload: {
                ...
                config: {
                    ...
                    color: {
                        type: 'string',
                        description: 'Arc color name. Empty uses the widget accent.'
                    }
                },
        ...
        render: function()
        {
            this.Compute(() =>
            {
                if(this.payload.color)
                {
                    this.tone = this.payload.color;
                }
                else
                {
                    this.tone = this.color ? this.color : 'brand';
                }
            });

            return `
                <e-admin-charts-gauge
                    :background="0"
                    :value="payload.value"
                    :min="payload.min"
                    :max="payload.max"
