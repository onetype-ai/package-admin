onetype.AddonReady('elements', (elements) =>
{
    elements.Item({
        ...
        render: function()
        {
            ...
            this.classes = (entry) => 'entry' + (entry.flat === this.index ? ' active' : '');
            ...
            this.key = ({ event }) =>
            {
                ...
                if(event.key === 'Enter')
                {
                    const entry = this.groups.flatMap((group) => group.results).find((result) => result.flat === this.index);
            ...
            return `
                <div class="box">
                    <div class="field">
                        <i>search</i>
                        <input :value="query" spellcheck="false" placeholder="Search apps, modes, pages, commands..." ot-input="input" ot-keydown="key">
                        <kbd>esc</kbd>
                    </div>
                    <div class="groups ot-scrollbar">
                        <div ot-for="group in groups" :ot-key="group.id" class="group">
                            <div class="name">{{ group.group }}</div>
                            <div ot-for="entry in group.results" :ot-key="entry.id" :class="classes(entry)" ot-click="run(entry)">
                                <i>{{ entry.icon }}</i>
